...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if (isModeChange > 0.0 && isAnimating > 0.0) {
        ...
        vec2 halfSizeCC = vec2(currentCursor.z, currentCursor.w) * 0.5;

        // distance from rectangle surface
        float distRect = getSdfRectangle(vu, centerCC, halfSizeCC);

        // propagate outward like the circle version
        float sdfRectRing = abs(distRect - rippleExpansion) - RING_THICKNESS * 0.5;
